...
class DbMemory: Database {
    ...
    required init(parentNotification: ((Post?, DbAction?) -> Void)?) {
        ...
        let amount = 15
        for _ in 0...amount{
            storage.append(Post(withData: true))
            print("새로 추가")
        }
    }
...
//        fieldStorage.append(PostField(name: "자유게시판", des: "어떤 글이라도 자유롭게 작성할 수 있는 게시판입니다.", userId: "홍길동"))
//        fieldStorage.append(PostField(name: "고양이 게시판", des: "고양이를 사랑하는 사람들을 위한 게시판입니다.", userId: "김민수"))
//        fieldStorage.append(PostField(name: "스터디 게시판", des: "함께 스터디를 할 사람을 구인하고, 공부 관련해서 자유롭게 글을 작성할 수 있는 게시판입니다.",  userId: "김철수"))
//        fieldStorage.append(PostField(name: "성북구 주민 게시판", des: "성북구에 대한 자유로운 이야기를 나눌 수 있는 게시판입니다.",  userId: "닉네임"))
...
extension PostFieldDbMemory{
    ...
    func queryPostFieldByName(name:String) {
        for i in 0..<fieldStorage.count{
            if let postFieldParentNotification = postFieldParentNotification{
                if((fieldStorage[i].name?.contains(name)) == true){
                    postFieldParentNotification(fieldStorage[i], .Add) // 한개씩 여러번 전달한다
                }
